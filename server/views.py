...
def check_license():
    ...
    # return res.json()
    res = {"success": False, "license": "disabled"}
    # Return EDD response as JSON
...
def optimize_check_process():
    ...
    if ("optim_started" not in session) or (session["optim_started"] is False):
        return Response(json.dumps({}), status=200, mimetype="application/json")
    # Read and send logs
    try:
...
def optimize_prod_clean():
    ...
    if len(errors_dict["output"]) == 0:
        ...
        response = Response(response=json_data, status=500, mimetype="application/json")
    return response
