...
def user_infos():
    ...
    try:
        license_exp = datetime.fromisoformat(license_exp).strftime("%a %d %b %Y")
    except Exception:
        pass
    # Prepare response
...
def delete_account():
    ...
    try:
        # Check password
        if not check_password_hash(current_user.password, request.json.get("password")):
            return "request_error", 400
...
def user_infos_license_opensankeyplus():
    ...
    """
    # Prepare response
    response = {"license_id": current_user.license_opensankeyplus}
    # Send back response
    return jsonify(response)


@connected_user.route("/user/infos/legacy/license_opensankeyplus", methods=["POST"])
@login_required
...
def user_infos_license_mfasankey():
    ...
    """
    # Prepare response
    response = {"license_id": current_user.license_sankeysuite}
    # Send back response
    return jsonify(response)


@connected_user.route("/user/infos/legacy/license_sankeysuite", methods=["POST"])
@login_required
...
def user_infos_is_developer():
    ...
    """
    # Prepare response
    response = {"is_dev": current_user.is_developer}
    # Send back response
    return jsonify(response)
...
def check_user_has_pref_dir():
   ...
   Function that check if user preference directory exist, \
    ...
    if current_user.dir is None:
        ...
        user_id: str = current_user.name.replace(" ", "_") + "".join(
...
def get_preference():
    ...
    try:
        ...
        # Send back response
        return Response(json.dumps(preference), status=200, mimetype="application/json")
    except Exception:
        trace.logger.error("get user preference failed")
        response = Response(json.dumps("{}"), status=500, mimetype="application/json")
        return response
