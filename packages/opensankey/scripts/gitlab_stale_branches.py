...
def get_branch_info(branch_name, repo_path=None, is_remote=False):
    ...
    for main_branch in main_branches:
        ...
        if run_command(f"git show-ref --verify --quiet refs/heads/{main_branch}", cwd=repo_path) is not None:
            continue
        if run_command(f"git show-ref --verify --quiet refs/remotes/origin/{main_branch}", cwd=repo_path) is not None:
            continue
...
def delete_branches_interactive(stale_branches, repo_path=None):
    ...
    for i, branch in enumerate(all_branches):
        ...
        while True:
            ...
            if choice in ['y', 'yes']:
                success = delete_branch(branch, repo_path, delete_remote=branch['is_remote'], delete_local=not branch['is_remote'])
                if success:
                    if branch['is_remote']:
                        deleted_remote += 1
                    else:
                ...
                if run_command(f"git checkout -b {branch['name']} origin/{branch['name']}", cwd=repo_path):
...
def delete_branch(branch_info, repo_path=None, delete_remote=False, delete_local=False):
        ...
        if not run_command(f"git branch -d {branch_name}", cwd=repo_path):
            ...
            if force.lower() in ['y', 'yes']:
                if run_command(f"git branch -D {branch_name}", cwd=repo_path):
                    ...
                    print(f"❌ Échec suppression locale forcée")
                    success = False
            else:
                success = False
        else:
    ...
    if delete_remote and success:
        ...
        if run_command(f"git push origin --delete {branch_name}", cwd=repo_path):
            ...
            print(f"❌ Échec suppression remote (permissions ? branche protégée ?)")
            success = False
...
def main():
    ...
    parser.add_argument('--stale-days', type=int, default=30, help='Nombre de jours pour considérer une branche comme ancienne')
    parser.add_argument('--show-only', action='store_true', help='Afficher seulement, ne pas supprimer')
    parser.add_argument('--category', choices=['orphaned', 'old', 'merged', 'remote_only'], help='Afficher seulement une catégorie')
