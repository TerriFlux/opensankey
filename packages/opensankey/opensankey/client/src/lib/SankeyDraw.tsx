...
const SankeyDraw : FunctionComponent<SankeyDrawTypes> = ({
  ...
  const add_links = (
    ...
    if (!static_sankey) {
      ...
      paths.call(d3.drag<SVGPathElement,SankeyLink>()
        ...
        .on('drag', function(event) {
          drag_link(nodes,links[region_name],display_style,data.selected_tags,this,event)
      ...
      select2.call(d3.drag<SVGTextElement,SankeyLink>()
        .subject(Object).on('drag', function(event) {
          if (alt_key_pressed) {
            ...
            drag_link(nodes,links[region_name],display_style,data.selected_tags,(d3.select(link_to_drag) .node() as SVGPathElement),event)
          }
        })
      )            
    }
    ...
    paths
      ...
      .attr('fill','none')
      .attr('stroke-opacity',d => d.visible && d.value >= display_style.filter ? 0.95 : 0)
      .attr('stroke-width', d => {   
        const link_value = test_link_value(nodes,d,data.selected_tags)  
    ...
    paths.attr('d', (d,i) => {
      setNodesHeight(nodes,links[region_name],d,data.selected_tags)
      return drawCurve(
        nodes,links[region_name],display_style,
        data.selected_tags,d,i,error_msg
      )
    })
      ...
      (link : SankeyLink, i : number ) => {
        if (link.source_name === node.name || link.target_name ===  node.name) {
          ...
          if (link.arrow) {
            ...
            drawArrows(target_node,nodes,links,display_style,selected_tags,node_select)
          }
  ...
  const drag_link = (
    ...
    if (linked_node.type === 'target' ) {
      ...
      drawArrows(node,nodes,links,display_style,selected_tags,node_select)
    }
  }
  ...
  const drawCurve = (
    ...
    if ( d.orientation === 'hh' ||  d.orientation === 'vv' ) {
      add_shift_handles(
        link_id,nodes,links,
        display_style,selected_tags,xs,ys,xt,yt
      )
    }
  ...
  const add_nodes_auto = (
    ...
    ggg_nodes
      ...
      .each( function(n) {
        ...
        drawArrows(n,nodes,links[region_name],display_style,data.selected_tags,selection)
      })
  ...
  const drawArrows = (
    ...
    for (let i=0;i<n.input_links.length;i++) {
      ...
      const link_value = test_link_value(nodes,l,selected_tags)
      if ( link_value === undefined ) {
        continue
      }
      ...
      if (!display_style.filter || link_value >= display_style.filter ) {
        selection
          .append('path')      
          .attr('d', () => {
            setNodeHeight(n,nodes,links,selected_tags)
