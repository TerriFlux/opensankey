...
for %%D in (node_modules dist build) do (
    if exist "!SCRIPT_DIR!\submodules\OpenSankey\opensankey\client\%%D" (
        echo removing !SCRIPT_DIR!\submodules\OpenSankey\opensankey\client\%%D
        rmdir /s /q "!SCRIPT_DIR!\submodules\OpenSankey\opensankey\client\%%D"
        if errorlevel 1 exit /b 1
    )
)
echo OK --------------------------------------------------------------------
