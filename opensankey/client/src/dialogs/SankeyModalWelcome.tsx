  ...
  return <>
    {list_template_data.map((_,idx) => {
      ...
      return(
        <Col key={idx}>
          <Card>
            <Card.Img className='img-card' variant="top" src={'/fm/userfiles/Formations/Démos/OpenSankey/image_preview/'+title+'.png'} style={{'objectFit':'contain','minHeight':'350px','maxHeight':'500px'}} />
            <Card.Body>
  ...
  const content_carousel=<Carousel variant='dark'> {
    carousel_img.map((_) => {
      ...
      return (<Carousel.Item>
        <img alt={title} src={'/fm/userfiles/Formations/Démos/OpenSankey/image_carousel/'+_}   style={{'objectFit':'contain','width':'100%','height':'650px',display:'inline-block'}}   />
      </Carousel.Item>)
    })
  }</Carousel>
