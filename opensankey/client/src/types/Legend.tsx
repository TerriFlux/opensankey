...
public fromJSON(json_object:{[_:string]:any}){
  ...
  /**
     * Function to draw tags in legend that are used in the sankey (when they're activated in the toolbar)
     *
     * @private
     * @memberof Class_Legend
     */
  ...
  private drawInfoDataType() {
    ...
    free_value?.append('text').attr('x', '35').text('MEP.show_legend_free_value').call(this.wrapper)
  }

  /**
     * Add text to describe why some link are dashed (because their value are undefined, only appear when data_type is set to anything but structur)
     *
     * @private
     * @memberof Class_Legend
     */
  private drawInfoDashedLink() {
    ...
    dashed_link?.append('text')
      ...
      .call(this.wrapper)
    dashed_link?.select('text').attr('x', '35').attr('y', this._legend_police / 2)
  }
