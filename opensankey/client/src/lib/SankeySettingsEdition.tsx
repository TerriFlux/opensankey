...
const SankeySettingsEditionTags: FunctionComponent<SankeyEditionTypes> = ({
  ...
  const { display_style, tags_catalog, links, node_width } = data
  ...
  const handleDelTag = () => {
    ...
    const length = tags_catalog[tag_group_id].tags.length
    if (tag_id == -1) {
      tags_catalog[tag_group_id].tags.splice(0, 1)
    } else {
      tags_catalog[tag_group_id].tags.splice(tag_id, 1)
    }
    set_data({ ...data })
  }
    ...
    <>     
      ...
      <Form.Group as={Row} >
        ...
        <Col>
          ...
          >
            {tags_catalog[tag_group_id].tags.map(
              (t, i) =>
                <option
                  key={i}
                  value={i}
