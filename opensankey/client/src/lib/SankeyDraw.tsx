...
const SankeyDraw: FunctionComponent<SankeyDrawTypes> = ({
  ...
  set_sub_nav_item_active = SankeyDrawDefaultProps.set_sub_nav_item_active,
  ...
  const min_width_and_height = () => {
    ...
    return [Math.max(width, window.innerWidth / 2), Math.max(height, window.innerHeight / 2)]
  }
  ...
  const add_nodes = (
    ...
    ggg_nodes.on('click', (event, d) => {
      if (!static_sankey && event.ctrlKey) {
        ...
        // set_nav_item_active('editionNoeud')
        set_show_nav(true)
      }
    })
