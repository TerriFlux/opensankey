...
const SankeyDraw: FunctionComponent<SankeyDrawTypes> = ({
  ...
  /* Test par ajout de zoom à la sourie*/
  /*
  (d3.select('#svg') as d3.Selection<SVGElement, any, null, undefined>)
    .call(d3.zoom()
      .extent([[0, 0], [500, 500]])
      .scaleExtent([1, 8])
      .on('zoom', zoomed)
  ...
  */
  ...
  const add_links = (
    ...
    console.log('Function : add_link')

    const defGradient = d3.select('#svg').append('defs').attr('id','sankey_def')

    const { nodes, links, display_style, region_name } = data
          ...
          links[region_name].forEach(
            (link: SankeyLink, i: number) => {
              d3.select('#link' + i).attr('d',
                () => {
                  return drawCurve(
                    nodes, links[region_name], display_style,
                    flux_types, link, i,
                    error_msg
                  )
                }
    ...
    // link value
    // Select -> pour les link_value dans les flux
    ...
    // Select2 pour les link_value des output
    ...
    select2
      ...
      .attr('id', d => 'link_value' + links[region_name].indexOf(d))
      .attr('class', 'link_value')
      .attr('style', 'font-weight: bold;font-family:Arial; font-size:' + display_style.font_size + 'px;text-anchor:middle')
      .attr('fill', d => d.text_color)
      .attr('visibility', d => link_visible(d))
    ...
    paths
      ...
      .style('stroke', function (d) {
        console.log(d)
        const id = d.source_name + '-' + d.target_name
        const gradient = defGradient.append('defs')
          ...
          .attr('x1', '0%')
          .attr('y1', '0%')
          .attr('x2', () => d.orientation=='hh'? '100%' : '0%')
          .attr('y2', () => d.orientation=='hh'? '0%' : '100%')
          .attr('spreadMethod', 'pad')

        gradient.append('stop')
          .attr('offset', '0%')
          .attr('stop-color', function() {return nodes.filter(elt => elt.name==d.source_name)[0].color})
          .attr('stop-opacity', 1)

        gradient.append('stop')
          .attr('offset', '100%')
          .attr('stop-color', function() {return nodes.filter(elt => elt.name==d.target_name)[0].color})
          .attr('stop-opacity', 1)
    ...
    // Creation du gradientLink
    // Sans doute a mettre dans le link même
    /*const gradientLink = d3.select('#g_links').selectAll('.gradient-link').data(links[region_name]).enter()
      .append('path')
      .attr('d', (d,i) => {
        setNodesHeight(nodes,links[region_name],d,flux_types)
        return drawCurve(
  ...
  const drag_node = (
    ...
    console.log('Function : drag_node')
    console.log('node qui est dragged : ', dragged)
    ...
    // newx, new_y : nouvelle position du node
    node.x = new_x
    node.y = new_y
    //console.log('display_style.trade_close = ', display_style.trade_close)

    // if (display_style.trade_close) {
    // Je ne comprends pas a quoi ca sert (peut être la gestion du tooltip)
    //   //console.log(node)
    //   // Boucle sur les links entrants
    //   node.input_links.forEach(
    //     i => {
    //       const l = links[i]
    //       const source_node = nodes.filter(n=> normalize_name(n.name) === normalize_name(l.source_name))[0]
    //       if (source_node.name.includes('(I') ) { // C'est quoi le (I
    ...
    links.forEach( // Pourquoi ne pas utiliser le  node.input_links et  node.output_links
      (link: SankeyLink, i: number) => {
        ...
        if (link.source_name === node.name || link.target_name === node.name) {
          ...
          // import_export : ca veut dire quoi
          console.log('import_export = ', import_export)
          if (!import_export) {
            if (!(link.label_position === 'frozen')) {
              // Je ne comprends pas a quoi ca sert, le link_value suit le path même sans
          ...
          if (link.arrow) {
            ...
            // il faut redessiner arrrows car elle peut changer de sens (si on passe a gauche du noeud)
  ...
  const drag_link = (
    ...
    console.log('Function drag_link')
  ...
  const drag_handle = (
    ...
    console.log('Function : frag_handle')
    const { width, height } = data
  ...
  const add_shift_handles = (
    ...
    for (let i = 0; i < shift_handles.length; i++) {
      ...
      if (selection.empty()) { // if the handle do not exist, create it
        add_shift_handle(
          link_id, nodes, links, display_style, flux_types, shift_handles[i][0], shift_handles[i][1]
        )
      }
    }
    for (let i = 0; i < shift_handles.length; i++) {
  ...
  const drawLinkText = (
    ...
    const positions: { [label_position: string]: string[] } = {
      'frozen': ['50%', 'start'],
      'beginning': ['5%', 'start'],
      // Modification de start-> middle pour center le text au milieu du flux
      'middle': ['50%', 'middle'],
      'end': ['95%', 'end']
    }
    // console.log(d)
    if (d.label_position === 'frozen' && d.x_label || !d.label_on_path || d.label_on_path === undefined) {
      ...
      (d3.select('#link_value' + link_id) as d3.Selection<SVGSVGElement, SankeyLink, HTMLElement, SankeyLink>)
        //.attr('xlink:href', '#link' + link_id)
        .attr('startOffset', positions[d.label_position][0])
        .attr('text-anchor', positions[d.label_position][1])
        // .attr('visibility', d.label_visible ? 'visible' : 'hidden')
        //Création du balise tspan permet d'affecter un dy pour ajuster le label au milieu du flux
        // Par contre cela fait pêter les plombs au drag :(
        //.append('tspan')
        //.attr('dy','0.3em')
        .text(d => link_text(d, link_value, display_style))
  ...
  const drawCurve = (
    ...
    if (d.orientation === 'hh' || d.orientation === 'vv') {
      add_shift_handles(
        link_id, nodes, links,
        display_style, flux_types, xs, ys, xt, yt
      )
    }
  ...
  const drag_text = (
    ...
    console.log('Function : drag_text')
  ...
  const add_nodes_auto = (
    ...
    if (!static_sankey) {
      ...
      ggg_nodes
        ...
        .on('click', (event, d) => {
          console.log('CLICK on gg.node#')
          const nodeStart = 'n' + d.id
          console.log(d)
          d3.select('#svg')
          d3.select('#svg').selectAll('.arrow').style('fill','#cccccc')
          d3.select('#svg').selectAll('.link').style('stroke','#cccccc')
          d3.select('#svg').selectAll('.node').style('fill','#cccccc')
          const links = (d3.select('#svg').selectAll('.link') as d3.Selection<SVGElement, SankeyLink, HTMLElement, SankeyLink>)
            .filter(function (d) {
              //console.log(d)
              return d.source_name == nodeStart
            })
            .attr('fill', 'none')
            .each(function (d) {
              //d.totalLength = (d3.select('#'+d.id).node() as SVGGeometryElement).getTotalLength()
              d.totalLength = (this as SVGGeometryElement).getTotalLength()
              d3.select(this)
                .attr('stroke-dasharray', d.totalLength + ' ' + d.totalLength)
                .attr('stroke-dashoffset', d.totalLength)
            })
          ...
          // Pour revenir à l'état initial normalement il y a un .end mais :( )
          // links
          //   .transition()
          //   .delay(3000)
          ...
          //     console.log(i)
          //     //const test = data.links['no_region'][i].totalLength
          //     // console.log(test)
    ...
    d3.selectAll('.node').attr('id', (d, i) => {
      ...
      .attr('fill-opacity', d => {
        const [total_input_offset, total_output_offset] = computeTotalOffsets((d as SankeyNode), data.links[data.region_name], false)
      ...
      .on('mousemove', function (d, node) {
        ...
        if (!d.visible) {
          sankeyTooltip
            ...
            .html('test')
          //.html('<b>'+ node.name +'<b><br> Type : '+d.type)
          return
        }
        d3.select(this).attr('class', 'selected_node')
      })
    ...
    const select = ggg_nodes.append('text')
      ...
      .attr('visibility', n => node_visible(n))
      .attr('style', d => {
    ...
    if (!static_sankey) {
      ...
      //nodes[1].test = (gg_nodes.nodes())[1]
      //}
      //console.log(nodes[0].test)
      //console.log(gg_nodes.nodes()[0])
      //nodes[0].test=gg_nodes.nodes()[0]
      //console.log(nodes)
    }
  ...
  const drawArrows = (
    ...
    for (let i = 0; i < n.input_links.length; i++) {
      ...
      if (!display_style.filter || link_value >= display_style.filter) {
        ...
        console.log(d3.select('#' + l.id))

        setNodeHeight(n, nodes, links, flux_types)

        d3.select('#gg_' + l.id + ' .arrow').remove()
        d3.select('#gg_' + l.id)
          //selection
          .append('path')
          .attr('class', 'arrow')
          .attr('d', () => {
            ...
            // if (l.orientation === 'hh') {
            //   if (n.x-source_node.x < 0) {l.recycling=true} else {l.recycling=false}
            // }
          ...
          .attr('fill', function() {console.log(l) ; return nodes.filter(elt => elt.name==l.target_name)[0].color })
