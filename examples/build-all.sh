...
# Prepare SA client (node_modules + dist/) so examples can resolve the
# @terriflux/sankeyapplication file: link.
echo ""
echo "[PREP] $SA_CLIENT"
if [ "$FORCE" = "1" ] || [ ! -d "$SA_CLIENT/node_modules" ]; then
    (cd "$SA_CLIENT" && npm install --no-audit --no-fund --prefer-offline --legacy-peer-deps)
fi
(cd "$SA_CLIENT" && npm run dist)
