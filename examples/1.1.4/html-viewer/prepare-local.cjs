// Copy the latest local SankeyApplication build (main.<hash>.js / .css) into
// bundle.js / bundle.css next to index.html, so the HTML viewer can be served
// locally without depending on the GitLab Pages release.
//
// Usage:
//   1. Build the SankeyApplication client first:
...
if (!jsSrc) {
  console.error(
    'No build/static/js/main.<hash>.js found. Run `npm run build` in client first.'
  );
  process.exit(1);
}
